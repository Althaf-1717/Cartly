          ...
          <div className="bg-[#18181b] border border-zinc-800 rounded-3xl p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400">Items below minimum stock threshold</p>
                </div>
              </div>
              <Link href="/admin/products" className="text-xs text-yellow-400 hover:underline font-bold">
                Manage Stock →
              </Link>
            </div>
            ...
            <div className="space-y-3">
              {lowStockItems.slice(0, 4).map((item) => (
                <div
                  key={item.id}
                  className="p-3 bg-zinc-950 rounded-2xl border border-zinc-800 flex items-center justify-between text-xs"
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={item.images?.[0] || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=80&q=80'}
                      alt=""
                      className="w-10 h-10 object-cover rounded-xl bg-zinc-900 border border-zinc-800"
                    />
                    <div>
                  ...
                  <div className="text-right">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${item.stock <= 10 ? 'bg-red-500/20 text-red-400' : 'bg-yellow-400/20 text-yellow-400'}`}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400">Live order fulfillment updates</p>
                </div>
              </div>
              <Link href="/admin/orders" className="text-xs text-yellow-400 hover:underline font-bold">
                View All →
              </Link>
            </div>
            ...
            <div className="space-y-3">
              {orders.slice(0, 4).map((order) => (
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-black text-yellow-400">{order.orderNumber}</span>
                    ...
                    <p className="text-[11px] text-zinc-400 mt-0.5">{order.customerName}</p>
                  </div>

                  <div className="text-right">
                    <p className="font-black text-white">{formatCurrency(order.totalAmount)}</p>
