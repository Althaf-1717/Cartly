...
export default function OrderDetailsPage({ params: paramsPromise }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] flex items-center justify-center text-slate-900 dark:text-white">
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] flex flex-col items-center justify-center text-slate-900 dark:text-white p-6 space-y-3">
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-50 transition-colors duration-200">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 py-10 w-full space-y-6">
        <div className="flex items-center justify-between">
          <Link
            href="/account"
            className="text-xs text-slate-500 dark:text-slate-300/70 hover:text-orange-600 flex items-center gap-1.5 transition font-bold"
          >
        ...
        <div className="bg-white dark:bg-[#111111] border border-slate-200 dark:border-slate-800/50 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100 dark:border-slate-800/60">
            <div>
              <span className="text-[11px] text-slate-400 dark:text-slate-400/60 uppercase font-bold">Order Reference</span>
              <h1 className="text-xl font-mono font-black text-orange-700 dark:text-orange-400">{order.orderNumber}</h1>
              <p className="text-xs text-slate-500 dark:text-slate-300/70 mt-1">Placed on {formatDateTime(order.createdAt)}</p>
            </div>
            <div className="sm:text-right">
              <span className="text-[11px] text-slate-400 dark:text-slate-400/60 uppercase font-bold">Total Amount</span>
              <p className="text-xl font-black text-slate-900 dark:text-white">{formatCurrency(order.totalAmount)}</p>
              <span className="text-[11px] text-orange-700 dark:text-orange-400 font-bold flex items-center gap-1 sm:justify-end mt-0.5">
                <ShieldCheck className="w-3.5 h-3.5 text-orange-600" /> Razorpay SSL Verified
              </span>
            </div>
          </div>
          ...
          <div className="py-4 space-y-4">
                    ...
                    <span
                      className={`text-[11px] font-bold capitalize block ${
                        isCurrent
                          ...
                          : 'text-slate-400 dark:text-slate-400/60'
                      }`}
                    >
                      {step}
                    </span>
            ...
            {order.trackingNumber && (
              <div className="p-3.5 bg-orange-50 dark:bg-slate-900 border border-orange-200 dark:border-slate-800 rounded-xl text-xs flex items-center justify-between">
                <span className="text-slate-700 dark:text-slate-200">
                  Courier AWB: <strong className="text-orange-800 dark:text-slate-300 font-mono">{order.trackingNumber}</strong>
                </span>
                <span className="text-orange-700 dark:text-orange-400 font-bold flex items-center gap-1">
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800/60">
            ...
            {order.items?.map((item, idx) => (
              <div
                key={idx}
                className="p-3.5 bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-300/70 text-[11px]">{item.variantName} • SKU: {item.sku}</p>
                  </div>
                </div>

                <div className="text-right">
                  <p className="text-slate-500 dark:text-slate-300/70">{item.quantity} × {formatCurrency(item.unitPrice)}</p>
          ...
          {order.shippingAddress && (
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800/60 text-xs">
              <h3 className="font-bold text-orange-700 dark:text-orange-400 uppercase tracking-wider mb-2">
                Shipping Destination
              </h3>
              <p className="text-slate-900 dark:text-white font-bold">{order.shippingAddress.fullName}</p>
              <p className="text-slate-600 dark:text-slate-200/80">{order.shippingAddress.street || order.shippingAddress.flatBuilding}</p>
              ...
              <p className="text-slate-500 dark:text-slate-400/60 mt-1">Phone: {order.shippingAddress.phone}</p>
            </div>
          )}
        </div>
      </main>
