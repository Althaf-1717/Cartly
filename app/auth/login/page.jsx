...
function LoginForm() {
  ...
  const handleCustomerLogin = async (e) => {
    ...
    setErrorMessage('');
    try {
      await login(custEmail, custPassword, 'customer', redirectUrl || null);
    } catch (err) {
  ...
  const handleAdminLogin = async (e) => {
    ...
    setErrorMessage('');
    try {
      await login(adminEmail, adminPassword, 'admin', redirectUrl || null);
    } catch (err) {
    ...
    <div className="w-full max-w-md bg-white dark:bg-[#111111] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
      ...
      {redirectUrl && (
        <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-xl text-xs text-orange-600 dark:text-orange-400 font-bold flex items-center gap-2">
          ...
          <span>Sign in to proceed directly to your selected product!</span>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleCustomerLogin} className="space-y-3 text-xs">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white font-bold rounded-xl shadow-md shadow-orange-600/20 transition flex items-center justify-center gap-2 mt-1"
            >
      ...
      <div className="text-center text-xs text-slate-500 dark:text-slate-400/60 pt-3 border-t border-slate-200 dark:border-slate-800/60">
        ...
        <Link
          href={
            redirectUrl
              ? `/auth/signup?role=${activeTab}&redirect=${encodeURIComponent(redirectUrl)}`
              : `/auth/signup?role=${activeTab}`
          }
          className="text-orange-600 dark:text-orange-400 font-bold hover:underline"
        >
