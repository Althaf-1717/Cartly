...
function getLocalData(key, fallback) {
  ...
  try {
    ...
    // Auto-refresh if seed data has more products or updated structure
...
export const StoreService = {
  ...
  async getOrders() {
    ...
    // Ensure every order has a verificationCode
    let updated = false;
  ...
  async createOrder(orderPayload) {
    ...
    const newOrder = {
      ...
      verificationCode,
      createdAt: new Date().toISOString(),
      status: orderPayload.status || 'booked',
    ...
    this.logAction(
      'ORDER_PLACED',
      `New order ${newOrder.orderNumber} placed for ₹${newOrder.totalAmount} by ${newOrder.customerName} (Verification: ${verificationCode})`
    );
    return newOrder;
  },
  ...
  async verifyOrderCode(orderId, code) {
    ...
    if (cleanInput === cleanActual) {
      ...
      saveLocalData(STORAGE_KEYS.ORDERS, orders);
      this.logAction('VERIFY_ORDER_CODE', `Order ${order.orderNumber} code verified successfully (${cleanActual})`);
      return { success: true, message: 'Verification Code Confirmed! Order status set to Booked.', order };
    }
  ...
  async deleteCustomer(id) {
    const customers = getLocalData(STORAGE_KEYS.CUSTOMERS, initialCustomers);
    const filtered = customers.filter((c) => c.id !== id && c.email !== id);
    saveLocalData(STORAGE_KEYS.CUSTOMERS, filtered);
    this.logAction('DELETE_CUSTOMER', `Deleted customer ID/Email: ${id}`);
    return true;
  },
